...
如果不加分号,报错的行号往往会是下一个语句的开头行号.
ks不像js会自动为你加分号,因此请养成加分号的好习惯(在任何现代编程语言都一样).
...
一个常量无法更改, 其内容也无法更改. 就连@index_set之类的setter方法也无法使用. 

但'无法修改'是定义在表达式上的(实际上只是约束了你对属性和索引内容修改的权限), 对于list.push之类的修改方法将不受影响
...
变量锁定后将拥有和常量相同的特性, 除非复制到另一变量, 否则将永远无法更改. 
