    ...
    pub fn evil(&mut self, code: &Stmt) {
        match code {
            ...
            Stmt::Class(cls) => {
                ...
                let clsdef = Box::into_raw(Box::new(ClassDef { p: *cls, cx: *self }));
                self.class_uses
            ...
            Stmt::Match { to, arms, def } => {
                ...
                let matcher =
                    |(val, ord): &(Expr, MatchOrd)| match (*to).partial_cmp(&*self.calc_ref(val)) {
                        Some(Ordering::Equal) => match ord {
                            MatchOrd::Eq | MatchOrd::GreaterEq | MatchOrd::LessEq => true,
                            _ => false,
                            MatchOrd::Greater => todo!(),
                            MatchOrd::Less => todo!(),
                        },
        ...
        AssignTo::Destr(ids) => match v {
            ...
            Litr::Inst(mut inst) => {
                ...
                for id in ids {
                    let i = cls
                        ...
                        .unwrap_or_else(|| panic!("本地类'{}'实例没有'{}'属性", cls.name, id));
            ...
            Litr::Obj(mut map) => {
                for id in ids {
                    let v = map
                        .remove(id)
                        .unwrap_or_else(|| panic!("哈希表中没有'{id}'属性"));
                    s.vars.push(Variant {
                        name: *id,
                        v,
                        locked,
...
/// 在一个作用域开始循环
fn start_loop(mut this: Scope, mut condition: impl FnMut() -> bool, exec: &Stmt) {
