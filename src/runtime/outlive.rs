...
pub fn scope_end(mut scope: Scope) {
    ...
    pub fn drop_func(v: &crate::primitive::litr::Litr) {
        ...
        match v {
            Litr::Func(Function::Local(f)) => {
                // println!("{:02}: func drop by end : {:p}",ln(), f.ptr);
                decrease_scope_count(f.scope);
            }
            Litr::Func(f) => {}
            Litr::List(l) => l.iter().for_each(drop_func),
            Litr::Inst(inst) => inst.v.iter().for_each(drop_func),
            Litr::Obj(map) => map.values().for_each(drop_func),
            _ => (),
        }
    }
