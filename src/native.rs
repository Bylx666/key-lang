...
static FUNCTABLE: FuncTable = FuncTable {
  ...
  err: |s| panic!("{}", s),
  find_var: Scope::var,
  let_var: |mut cx, name, v| {
    cx.vars.push(Variant {
      locked: false,
      name,
      v,
    })
  ...
  using: |mut cx, name, cls| {
    cx.class_uses
      .push((name, crate::runtime::Class::Native(cls)))
  },
...
pub fn parse(path: &[u8]) -> *const NativeMod {
  ...
  unsafe {
    ...
    // 没有extern前缀!
    let premain: fn(&FuncTable) = std::mem::transmute(lib.get(b"premain").expect(
      "请为你的项目添加'key_native'库. 入门教程请参见: https://docs.subkey.top/native/1.start",
    ));
    premain(&FUNCTABLE);
