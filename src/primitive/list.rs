...
fn insert_many(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    ...
    .expect("list.insert_many需要传入第二个参数作为插入内容")
  {
    Litr::Buf(b) => {
      let _ = v
        .splice(index..index, b.iter().map(|n| Litr::Uint(*n as usize)))
        .collect::<Vec<_>>();
    }
    Litr::List(b) => {
      let _ = v
        .splice(index..index, b.iter().map(|n| n.clone()))
        .collect::<Vec<_>>();
    }
...
fn splice(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
  ...
  Litr::List(match &**args.get(2).unwrap() {
    Litr::Buf(b) => v
      ...
      .collect(),
    Litr::List(b) => v.splice(start..end, b.iter().map(|n| n.clone())).collect(),
    n => v.splice(start..end, [n.clone()].into_iter()).collect(),
  })
}
...
fn join(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
  ...
  for n in &mut v[1..] {
    if let Litr::Str(s) = n {
      ...
      res
        .write_fmt(format_args!("{sep}{}", n.str()))
        .expect(WRITE_ERR);
    }
  }
  Litr::Str(res)
