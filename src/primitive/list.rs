...
fn push_front(v: &mut Vec<Litr>, mut args: Vec<CalcRef>) -> Litr {
    ...
    new_v.push(
        args.get_mut(0)
            .expect("list.push_front需要一个要推进的元素")
            .take(),
    );
...
fn dedup(v: &mut Vec<Litr>, args: Vec<CalcRef>, scope: Scope) -> Litr {
    ...
    if let Some(f) = args.first() {
        ...
        v.dedup_by(
            |a, b| match scope.call(vec![CalcRef::Ref(a), CalcRef::Ref(b)], f) {
                Litr::Bool(b) => b,
                _ => false,
            },
        );
...
/// 排序
fn sort(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    // 如果传了第一个参数就用sort_by
    if let Some(f) = args.first() {
        let f = match &**f {
            Litr::Func(f) => f,
        ...
        v.sort_unstable_by(|a, b| {
            match scope.call(vec![CalcRef::Own(a.clone()), CalcRef::Own(b.clone())], f) {
                Litr::Bool(b) => match b {
...
/// 循环调用
fn for_each(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("list.foreach需要一个函数作为参数") {
        Litr::Func(f) => f,
        _ => panic!("list.foreach第一个参数只能传函数"),
    };
    v.iter_mut().for_each(|a| {
        scope.call(vec![CalcRef::Ref(a)], f);
    });
    Litr::Uninit
}

/// 映射重构新Buf
fn map_clone(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("list.map需要一个函数作为参数") {
        Litr::Func(f) => f,
        _ => panic!("list.map第一个参数只能传函数"),
    };
    Litr::List(
        v.iter_mut()
            .map(|a| scope.call(vec![CalcRef::Ref(a)], f))
            .collect(),
    )
}
...
/// 反转Buf
fn rev(v: &mut [Litr]) -> Litr {
    v.reverse();
    Litr::Uninit
}

/// filter的复制版本
fn filter_clone(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("list.filter需要一个函数作为参数") {
        Litr::Func(f) => f,
        _ => panic!("list.map第一个参数只能传函数"),
    };
    ...
    Litr::List(
        v.iter_mut()
            .filter_map(|a| match scope.call(vec![CalcRef::Ref(a)], f) {
...
fn insert(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    ...
    let index = to_usize(
        &args
            .next()
            .expect("list.insert需要传入一个数字作为插入位置"),
    );
    assert!(
        index <= v.len(),
        "插入索引{index}不可大于数组长度{}",
    ...
    let to_insert = args
        .next()
        .expect("list.insert需要传入第二个参数作为插入内容")
        .own();
...
fn insert_many(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    let index = to_usize(
        args.first()
            .expect("list.insert_many需要传入一个数字作为插入位置"),
    );
    assert!(
        index <= v.len(),
        "插入索引{index}不可大于数组长度{}",
    ...
    match &**args
        .get(1)
        .expect("list.insert_many需要传入第二个参数作为插入内容")
    {
        Litr::Buf(b) => {
            v.splice(index..index, b.iter().map(|n| Litr::Uint(*n as usize)))
                .collect::<Vec<_>>();
        }
        Litr::List(b) => {
            v.splice(index..index, b.iter().cloned())
                .collect::<Vec<_>>();
        }
...
fn remove(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    let index = to_usize(args.first().expect("list.remove需要一个整数作为删除索引"));
    assert!(
        index < v.len(),
...
fn splice(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    assert!(
        args.len() >= 3,
        "list.splice需要3个参数:删除起始索引,删除结束索引,要插入的列表或数组"
    );
    ...
    Litr::List(match &**args.get(2).unwrap() {
        Litr::Buf(b) => v
            .splice(start..end, b.iter().map(|n| Litr::Uint(*n as usize)))
            .collect(),
        Litr::List(b) => v.splice(start..end, b.iter().cloned()).collect(),
        n => v.splice(start..end, [n.clone()]).collect(),
    })
}

/// 将一片区域填充为指定值
fn fill(v: &mut [Litr], args: Vec<CalcRef>) -> Litr {
    let mut args = args.into_iter();
...
fn expand(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    let n = to_usize(args.first().expect("list.expand需要一个整数作为扩大字节数"));
    v.reserve(n);
    Litr::Uninit
}

/// 横向旋转数组, 相当于整体移动并将溢出值移到另一边
fn rotate(v: &mut [Litr], args: Vec<CalcRef>) -> Litr {
    let mut n = to_usize(args.first().expect("list.rotate需要一个整数代表移动字节数"));
    // 使旋转大小永小于数组长度
    n %= v.len();
    // 如果第二个参数传了true就左移
...
fn concat(v: &mut Vec<Litr>, args: Vec<CalcRef>) -> Litr {
    let mut args = args.into_iter();
    let other: Vec<Litr> = match args
        .next()
        .expect("list.concat需要传入另一个Buf或数组")
        .own()
    {
        Litr::List(b) => b,
...
fn join(v: &mut [Litr], args: Vec<CalcRef>) -> Litr {
    if v.is_empty() {
...
/// 嘎嘎复制和计算, 将整个数组折叠成一个值
fn fold(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let mut args = args.into_iter();
    let mut init = args.next().expect("list.fold需要一个初始值").clone().own();
    let f_ = args
        .next()
        .expect("list.fold需要第二个参数的函数来处理数据");
    let f = match &*f_ {
        Litr::Func(f) => f,
...
/// slice
fn slice_clone(v: &mut [Litr], args: Vec<CalcRef>) -> Litr {
    let len = v.len();
...
/// 是否存在一个数
fn includes(v: &mut [Litr], args: Vec<CalcRef>) -> Litr {
    let find = args.first().expect("list.includes需要知道你要找啥");
    Litr::Bool(v.iter().any(|n| n == &**find))
}

/// 找数组中第一个所指数字, 也可以传函数来自定义判断
fn index_of(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let find = &**args.first().expect("list.index_of需要传入一个值");
...
/// index_of反向版
fn r_index_of(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let find = &**args.first().expect("list.r_index_of需要传入一个值");
...
/// 通过判断函数找到第一个对应值
fn find(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let find = &**args.first().expect("list.find需要传入一个函数");
    let res = if let Litr::Func(f) = find {
        v.iter_mut()
            .position(|n| match scope.call(vec![CalcRef::Ref(n)], f) {
                Litr::Bool(n) => n,
                _ => false,
            })
    } else {
        panic!("list.find第一个参数必须是函数")
    };
    res.map_or(Litr::Uninit, |n| v[n].clone())
}

/// 通过判断函数找到第一个对应值
fn r_find(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let find = &**args.first().expect("list.r_find需要传入一个函数");
    let res = if let Litr::Func(f) = find {
        v.iter_mut()
            .rev()
            .position(|n| match scope.call(vec![CalcRef::Ref(n)], f) {
        ...
        panic!("list.r_find第一个参数必须是函数")
    };
    res.map_or(Litr::Uninit, |n| v[v.len() - n - 1].clone())
}

/// 测试所有元素是否都能让传入函数返回true
fn all(v: &mut [Litr], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args
        .first()
        .expect("list.all需要传入一个函数来判断元素是否所需")
    {
        Litr::Func(f) => f,
    ...
    let b = v
        .iter_mut()
        .all(|n| match scope.call(vec![CalcRef::Ref(n)], f) {
            Litr::Bool(b) => b,
            _ => false,
        });
    Litr::Bool(b)
...
/// 找最小值
fn min(v: &mut [Litr]) -> Litr {
    v.iter()
        .cloned()
        .min_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
        .unwrap_or(Litr::Uninit)
}

/// 找最大值
fn max(v: &mut [Litr]) -> Litr {
    v.iter()
        .cloned()
...
fn s_from_iter(mut args: Vec<CalcRef>, _cx: Scope) -> Litr {
    let from = args
        .get_mut(0)
        .expect("List::from_iter需要一个可迭代的元素");
    Litr::List(iter::LitrIterator::new(from).collect())
}

/// 通过buf创建List,相当于buf.to_list
fn s_from_buf(args: Vec<CalcRef>, _cx: Scope) -> Litr {
    let from = args.first().expect("List::from_buf需要一个Buf");
    if let Litr::Buf(v) = &**from {
        Litr::List(v.iter().map(|n| Litr::Uint(*n as usize)).collect())
    } else {
        panic!("List::from_buf第一个参数必须是Buf")
    }
}
