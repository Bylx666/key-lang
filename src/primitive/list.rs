...
/// 推进单个元素
fn push(v:&mut Vec<Litr>, args:Vec<CalcRef>)-> Litr {
  let e = args.get(0).expect("list.push需要一个要推进的元素");
  v.push(e.own());
  Litr::Uninit
}

/// 像是js的unshift
fn push_front(v:&mut Vec<Litr>, args:Vec<CalcRef>)-> Litr {
  let e = args.get(0).expect("list.push_front需要一个要推进的元素");
  *v = [&[e.own()], &**v].concat();
  Litr::Uninit
}
