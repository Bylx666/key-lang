...
fn set(v: &mut HashMap<Interned, Litr>, args: Vec<CalcRef>) -> Litr {
    let name = match &**args.first().expect("obj.insert需要传入键名") {
...
fn remove(v: &mut HashMap<Interned, Litr>, args: Vec<CalcRef>) -> Litr {
    let name = match &**args.first().expect("obj.remove需要传入键名") {
        Litr::Str(s) => intern(s.as_bytes()),
...
fn for_each(v: &mut HashMap<Interned, Litr>, args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("obj.for_each需要传入一个函数") {
        Litr::Func(f) => f,
...
fn get(v: &mut HashMap<Interned, Litr>, args: Vec<CalcRef>) -> Litr {
    let name = match &**args.first().expect("obj.get需要传入键名") {
        Litr::Str(s) => intern(s.as_bytes()),
...
fn has(v: &mut HashMap<Interned, Litr>, args: Vec<CalcRef>) -> Litr {
    let name = match &**args.first().expect("obj.has需要传入键名") {
        Litr::Str(s) => intern(s.as_bytes()),
        _ => panic!("obj.has第一个参数必须是字符串"),
    };
    Litr::Bool(v.get(&name).is_some())
}
...
fn _concat_extend(o: &mut HashMap<Interned, Litr>, arg: &Litr) {
    match arg {
        Litr::Obj(other) => o.extend(other.iter().map(|(k, v)| (*k, v.clone()))),
        Litr::Inst(inst) => {
...
fn concat(o: &mut HashMap<Interned, Litr>, args: Vec<CalcRef>) -> Litr {
    _concat_extend(o, args.first().expect("obj.concat需要传入拼接对象或实例"));
    Litr::Uninit
}
    ...
    unsafe {
        ...
        ITER_KEYS = Box::into_raw(Box::new(super::new_iter_class(
            ...
            |v| {
                ...
                (*itr)
                    .next()
                    .map_or(sym::iter_end(), |v| Litr::Str(v.str()))
            },
            |v| drop(Box::from_raw(v.v as *mut Keys<'_, Interned, Litr>)),
        )));
        ...
        ITER_VALUES = Box::into_raw(Box::new(super::new_iter_class(
            ...
            |v| {
                ...
                (*itr).next().map_or(sym::iter_end(), |v| v.clone())
            },
            |v| drop(Box::from_raw(v.v as *mut Values<'_, Interned, Litr>)),
        )));
        ...
        ITER_ENTRIES = Box::into_raw(Box::new(super::new_iter_class(
            ...
            |v| {
                ...
                (*itr).next().map_or(sym::iter_end(), |(k, v)| {
                    Litr::List(vec![Litr::Str(k.str()), v.clone()])
                })
            },
            |v| drop(Box::from_raw(v.v as *mut Iter<'_, Interned, Litr>)),
        )));
    }
...
fn s_from_list(args: Vec<CalcRef>, _cx: Scope) -> Litr {
    let l = match &**args.first().expect("Obj::from_list需要传入一个List") {
        Litr::List(l) => l,
...
fn s_group_by(args: Vec<CalcRef>, cx: Scope) -> Litr {
    assert!(
        args.len() >= 2,
        "Obj::group_by需要一个List和返回字符串的函数"
    );
    let mut args = args.into_iter();
    let mut o = HashMap::new();
    let mut ls_ = args.next().unwrap();
    let ls = if let Litr::List(l) = &mut *ls_ {
        l
    } else {
        panic!("Obj::group_by第一个参数必须是List")
    };
    let f_ = args.next().unwrap();
    let f = if let Litr::Func(f) = &*f_ {
        f
    } else {
        panic!("Obj::group_by第二个参数必须是返回字符串的Func")
    };

    for elem in ls.iter_mut() {
        let sort_str = cx.call(vec![CalcRef::Ref(elem)], f);
        let s = intern(if let Litr::Str(s) = &sort_str {
            s.as_bytes()
        } else {
            continue;
        });
...
fn s_new(args: Vec<CalcRef>, _cx: Scope) -> Litr {
    Litr::Obj(if let Some(n) = args.first() {
