...
fn push(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    ...
    .expect("'push'方法需要一个数字,列表或数组作为参数")
  {
    ...
    Litr::List(right) => {
      v.extend_from_slice(&right.iter().map(|litr| to_u8(litr)).collect::<Box<[u8]>>())
    }
    n => v.push(to_u8(n)),
  };
  Litr::Uninit
...
fn insert(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    ...
    .expect("buf.insert需要传入第二个参数:整数,列表或数组作为插入内容")
  {
    Litr::Buf(b) => {
      let _ = v
        .splice(index..index, b.iter().copied())
        .collect::<Vec<_>>();
    }
    Litr::List(b) => {
      let _ = v
        .splice(index..index, b.iter().map(|n| to_u8(n)))
        .collect::<Vec<_>>();
    }
...
fn splice(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
  ...
  Litr::Buf(match &**args.get(2).unwrap() {
    ...
    Litr::List(b) => v.splice(start..end, b.iter().map(|n| to_u8(n))).collect(),
    n => {
      let n = to_u8(n);
...
fn includes(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
  Litr::Bool(
    match &**args.get(0).expect("buf.includes需要知道你要找啥") {
