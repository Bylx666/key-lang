...
fn push(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    match &**args
        .first()
        .expect("'push'方法需要一个数字,列表或数组作为参数")
    {
        Litr::Buf(right) => v.extend_from_slice(right),
        Litr::List(right) => v.extend_from_slice(&right.iter().map(to_u8).collect::<Box<[u8]>>()),
        n => v.push(to_u8(n)),
    };
    Litr::Uninit
}
...
fn push_front(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    match &**args
        .first()
        .expect("'push_front'方法需要一个数字,列表或数组作为参数")
    {
        Litr::Buf(right) => *v = [&**right, v].concat(),
        Litr::List(right) => *v = [&*right.iter().map(to_u8).collect::<Box<[u8]>>(), v].concat(),
        ...
        v.dedup_by(|a, b| {
            match scope.call(
                vec![
                    ...
                    CalcRef::Own(Litr::Uint(*b as usize)),
                ],
                f,
            ) {
                Litr::Bool(b) => b,
...
/// 排序
fn sort(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    // 如果传了第一个参数就用sort_by
    if let Some(f) = args.first() {
        let f = match &**f {
            Litr::Func(f) => f,
...
/// 循环调用
fn for_each(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("buf.foreach需要一个函数作为参数") {
        Litr::Func(f) => f,
        _ => panic!("buf.foreach第一个参数只能传函数"),
    };
    v.iter().for_each(|a| {
        scope.call(vec![CalcRef::Own(Litr::Uint(*a as usize))], f);
    });
    Litr::Uninit
}

/// 映射重构新Buf
fn map_clone(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("buf.map需要一个函数作为参数") {
        Litr::Func(f) => f,
    ...
    Litr::Buf(
        v.iter()
            .map(
                |a| match scope.call(vec![CalcRef::Own(Litr::Uint(*a as usize))], f) {
                    Litr::Uint(n) => n as u8,
                    Litr::Int(n) => n as u8,
                    _ => 0,
                },
...
/// 反转Buf
fn rev(v: &mut [u8]) -> Litr {
    v.reverse();
    Litr::Uninit
}

/// filter的复制版本
fn filter_clone(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args.first().expect("buf.filter需要一个函数作为参数") {
        Litr::Func(f) => f,
        _ => panic!("buf.map第一个参数只能传函数"),
    };
...
fn copy_within(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    assert!(
        args.len() >= 3,
        "buf.copy_within需要传入3个参数:起始索引,结束索引,复制目标索引"
    );
    let start = to_usize(args.first().unwrap());
...
fn write(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    ...
    assert!(
        index < ori_len,
        "传入索引{index}不可大于等于数组长度{ori_len}"
    );

    let ls_tmp;
    let buf = match &**args
        .next()
        .expect("buf.write需要一个列表或数组作为写入内容")
    {
        Litr::List(ls) => {
            ls_tmp = ls.iter().map(to_u8).collect();
            &ls_tmp
        }
        Litr::Buf(b) => b,
...
/// 获取最后一个数字
fn last(v: &mut [u8]) -> Litr {
    v.last().map_or(Litr::Uninit, |v| Litr::Uint(*v as usize))
}

/// repeat的复制版
fn repeat_clone(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let n = to_usize(args.first().expect("buf.repeat需要传入整数作为重复次数"));
    Litr::Buf(v.repeat(n))
}
...
fn insert(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    ...
    assert!(
        index < v.len(),
        "插入索引{index}不可大于等于数组长度{}",
        v.len()
    );

    match &**args
        .next()
        .expect("buf.insert需要传入第二个参数:整数,列表或数组作为插入内容")
    {
        Litr::Buf(b) => {
            v.splice(index..index, b.iter().copied())
                .collect::<Vec<_>>();
        }
        Litr::List(b) => {
            v.splice(index..index, b.iter().map(to_u8))
                .collect::<Vec<_>>();
        }
...
fn remove(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    let index = to_usize(args.first().expect("buf.remove需要一个整数作为删除索引"));
    assert!(
        index < v.len(),
        "删除索引{index}不可大于等于数组长度{}",
        v.len()
    );
...
fn splice(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    assert!(
        args.len() >= 3,
        "buf.splice需要3个参数:删除起始索引,删除结束索引,要插入的内容(数组或整数)"
    );
    let start = to_usize(args.first().unwrap());
    let end = to_usize(args.get(1).unwrap());
    assert!(start <= end, "起始索引{start}不可大于结束索引{end}");
    ...
    Litr::Buf(match &**args.get(2).unwrap() {
        Litr::Buf(b) => v.splice(start..end, b.iter().copied()).collect(),
        Litr::List(b) => v.splice(start..end, b.iter().map(to_u8)).collect(),
        n => {
            let n = to_u8(n);
...
/// 将一片区域填充为指定值
fn fill(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let mut args = args.iter();
...
fn expand(v: &mut Vec<u8>, args: Vec<CalcRef>) -> Litr {
    let n = to_usize(args.first().expect("buf.expand需要一个整数作为扩大字节数"));
    v.reserve(n);
    Litr::Uninit
}

/// 横向旋转数组, 相当于整体移动并将溢出值移到另一边
fn rotate(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let mut n = to_usize(args.first().expect("buf.rotate需要一个整数代表移动字节数"));
    // 使旋转大小永小于数组长度
    n %= v.len();
    // 如果第二个参数传了true就左移
...
fn join(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    if v.is_empty() {
        return Litr::Str(String::new());
    }
...
/// 嘎嘎复制和计算, 将整个数组折叠成一个值
fn fold(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let mut init = args.first().expect("buf.fold需要一个初始值").clone().own();
    ...
    v.iter().fold(init, |a, b| {
        scope.call(
            vec![CalcRef::Own(a), CalcRef::Own(Litr::Uint(*b as usize))],
            f,
        )
    })
}
...
/// slice的复制版本
fn slice_clone(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let len = v.len();
    let start = args.first().map_or(0, |n| to_usize(n));
    let end = args.first().map_or(len, |n| to_usize(n));
...
/// 是否存在一个数
fn includes(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    Litr::Bool(
        match &**args.first().expect("buf.includes需要知道你要找啥") {
...
fn index_of(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let res = match &**args
        .first()
        .expect("buf.index_of需要传入一个数字或判断函数")
    {
        Litr::Func(f) => v.iter().position(|n| {
...
/// index_of的反向版本
fn r_index_of(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let res = match &**args.first().expect("buf.r_index_of需要知道你要找啥") {
        Litr::Func(f) => v.iter().rev().position(|n| {
            match scope.call(vec![CalcRef::Own(Litr::Uint(*n as usize))], f) {
                Litr::Bool(n) => n,
                _ => false,
            }
...
fn all(v: &mut [u8], args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args
        .first()
        .expect("buf.all需要传入一个函数来判断元素是否所需")
    {
        Litr::Func(f) => f,
        _ => panic!("buf.all第一个参数必须是函数"),
    };
    let b = v.iter().all(
        |n| match scope.call(vec![CalcRef::Own(Litr::Uint(*n as usize))], f) {
            Litr::Bool(b) => b,
            _ => false,
        },
    );
...
fn part(v: &mut Vec<u8>, args: Vec<CalcRef>, scope: Scope) -> Litr {
    let f = match &**args
        .first()
        .expect("buf.part需要传入一个函数来判断元素是否所需")
    {
        Litr::Func(f) => f,
...
fn read(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let index = args.first().map_or(0, |n| to_usize(n));
    let sz = args.get(1).map_or(8, |n| to_usize(n));
    let big_endian = args.get(2).map_or(false, |n| match &**n {
        Litr::Bool(b) => *b,
        _ => false,
    });
...
fn read_float(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let index = args.first().map_or(0, |n| to_usize(n));
    if index + 8 > v.len() {
        return Litr::Float(0.0);
    }
    let big_endian = args.get(1).map_or(false, |n| match &**n {
        Litr::Bool(b) => *b,
        _ => false,
    });
...
/// replace
fn replace_clone(v: &mut [u8], args: Vec<CalcRef>) -> Litr {
    let s = to_str(v);

    let from = match &**args.first().expect("buf.replace需要一个搜索Buf") {
        Litr::Str(s) => s,
        Litr::Buf(s) => to_str(s),
...
fn s_from_list(args: Vec<CalcRef>, _cx: Scope) -> Litr {
    let ls = match &**args.first().expect("Buf::from_list需要传入一个列表") {
        Litr::List(ls) => ls,
        _ => panic!("Buf::from_list第一个参数必须是列表"),
    };
    Litr::Buf(ls.iter().map(to_u8).collect())
}
...
fn s_from_iter(mut args: Vec<CalcRef>, _cx: Scope) -> Litr {
    let from = args
        .get_mut(0)
        .expect("Buf::from_iter需要一个允许迭代的元素");
    let itr = iter::LitrIterator::new(from);
...
fn s_from_ptr(args: Vec<CalcRef>, _cx: Scope) -> Litr {
    ...
    unsafe { Litr::Buf(std::slice::from_raw_parts(from as *const u8, len).to_vec()) }
}

/// Buf::concat拼接两个Buf,允许传List自动转换
fn s_concat(args: Vec<CalcRef>, _cx: Scope) -> Litr {
    assert!(args.len() >= 2, "Buf::concat需要左右两个buf作参数");
    let mut left = match &**args.first().unwrap() {
        Litr::List(v) => v.iter().map(to_u8).collect(),
        Litr::Buf(v) => v.clone(),
        n => vec![to_u8(n)],
    };
    match &**args.get(1).unwrap() {
        Litr::List(v) => left.extend(v.iter().map(to_u8)),
