...
use crate::native::{NativeClassDef, NativeFn, NativeInstance};
use crate::runtime::{calc::CalcRef, Class, Scope};
use litr::{Function, Litr};
...
fn new_static_class(s: &[u8], f: Vec<(Interned, NativeFn)>) -> (Interned, *mut NativeClassDef) {
    ...
    (
        name,
        Box::into_raw(Box::new(NativeClassDef {
            name,
            methods: Vec::new(),
            statics: f,
...
/// 返回只含有静态函数的内置类
pub fn classes() -> Vec<(Interned, Class)> {
    unsafe {
        if let Some(cls) = unsafe { &mut *addr_of_mut!(CLASSES) } {
            cls.iter()
                .map(|(name, f)| (*name, Class::Native(*f)))
                .collect()
        } else {
            CLASSES = Some(vec![
