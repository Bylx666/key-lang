            ...
            b'`' => {
                            ...
                            match escaper {
                                ...
                                _ => {
                                    ...
                                    if escaped == 255 {
                                        panic!(
                                            "错误的转义符:{}",
                                            String::from_utf8_lossy(&[escaper])
                                        );
                                    }
                                    vec.push(escaped);
                                    i += 1;
                        ...
                        b'{' => {
                            ...
                            if let Some(last_e) = &mut expr_catch {
                                ...
                                let left = Box::new(Expr::Binary {
                                    left,
                                    right: Box::new(Expr::Literal(Litr::Str(
                                        String::from_utf8(part_vec).expect("字符串含非法字符"),
                                    ))),
                                    op: b"+".to_vec().into(),
                                });
                                // 再把上述expr和这次的捕获表达式相加
                                ...
                                expr_catch = Some(Expr::Binary {
                                    left: Box::new(Expr::Literal(Litr::Str(
                                        String::from_utf8(part_vec).expect("字符串含非法字符"),
                                    ))),
                                    right: this_e,
                                    op: b"+".to_vec().into(),
                ...
                if let Some(e) = &mut expr_catch {
                    ...
                    Expr::Binary {
                        left,
                        right: Box::new(Expr::Literal(Litr::Str(
                            String::from_utf8(std::mem::take(&mut vec)).expect("字符串含非法字符"),
                        ))),
                        op: b"+".to_vec().into(),
                    }
                } else {
                ...
                if self.cur() == b'0' {
                    match self.src[self.i() + 1] {
                        ...
                        b'x' => {
                            self.set_i(self.i() + 2);
                            let mut i = self.i();
                            while self.src[i].is_ascii_hexdigit() {
                                i += 1
                            }

                            let n = usize::from_str_radix(
                                &String::from_utf8_lossy(&self.src[self.i()..i]),
                                16,
                            )
                        ...
                        b'b' => {
                            self.set_i(self.i() + 2);
                            let mut i = self.i();
                            while self.src[i] == b'0' || self.src[i] == b'1' {
                                i += 1
                            }
