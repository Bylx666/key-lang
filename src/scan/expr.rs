use super::{charts, Scanner};
use crate::intern::{intern, Interned};
use crate::primitive::litr::{KsType, Litr, LocalFuncRaw};
...
impl Scanner<'_> {
    ...
    pub fn expr_with_left(&self, left: Expr, mut unary: Vec<u8>) -> Expr {
        ...
        loop {
            ...
            // 优先级够的话, 优先合并一元运算符
            if precedence < charts::PREC_UNARY && !unary.is_empty() {
          ...
          if last_op == $op {
            ...
            panic!("{}左侧需要一个标识符",String::from_utf8_lossy($op))
                ...
                if last_op == b"::" {
                    match right {
                        Expr::Variant(id) => expr_stack.push(Expr::ImplAccess(Box::new(left), id)),
                        Expr::Obj(o) => expr_stack.push(Expr::NewInst {
                            cls: Box::new(left),
                            val: o,
                        }),
                        _ => panic!("::右侧只能是标识符或对象"),
            ...
            match op {
                ...
                b"." => {
                    ...
                    if self.cur() == b'(' {
                        ...
                        expr_stack.push(Expr::CallMethod {
                            args,
                            targ: left,
                            name,
                        });
                ...
                b"[" => {
                    ...
                    self.next();
                    expr_stack.push(Expr::Index { left, i });
                    continue;
                }
                ...
                b"|>" => {
                    unsafe {
                        ON_PIPE = Some(expr_stack.pop().unwrap());
                    }
