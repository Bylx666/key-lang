...
use crate::runtime::{Module, Scope, ScopeInner};
use crate::scan::Expr;
use crate::LINE;
...
impl Scanner<'_> {
        ...
        match sym {
            ...
            b'(' => {
                ...
                AssignDef {
                    id,
                    take: false,
                    val: Expr::LocalDecl(LocalFuncRaw {
                        argdecl: args,
                        stmts,
                    }),
                }
    ...
    fn moding(&self) -> Stmt {
        ...
        match suffix {
            b".ksm" | b".dll" => unsafe {
                /// 让mod过程出错时知道是原生模块的锅
                let mut place = std::mem::take(unsafe { &mut *addr_of_mut!(crate::PLACE) });
            ...
            b".ks" => {
                let file =
                    std::fs::read(&*path).unwrap_or_else(|e| panic!("无法找到模块'{}'", path));
                unsafe {
                    // 将报错位置写为该模块 并保存原先的报错数据
                    let mut place = std::mem::take(unsafe { &mut *addr_of_mut!(crate::PLACE) });
                    crate::PLACE.clone_from(&path);
                    let line = crate::LINE;
                    crate::LINE = 1;
